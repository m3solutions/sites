...
//$type = 'excel';
 ...
 switch ($dados['type']) {
                ...
                case '1':  // leads por por produto 
                        {
//pega id do produto 
                    $type = 'excel';
                    $sql = "select * from {$dbPre}leadType where typeName='{$dados['produt']}'";
                            $iddoproduto = $db->extQuery($sql);
        
                            foreach ($iddoproduto as $caraio) {
                                $produtoid1 =  $caraio->typeID;
                            }
...
//$sql = "select c.*, ls.sourceName, lst.statusName, lt.typeName"
 //    . " from {$dbPre}contacts c, {$dbPre}leadSource ls, {$dbPre}leadStatus lst, {$dbPre}leadType lt"
  //   . " where c.leadType=lt.typeID and c.leadSource=ls.sourceID and c.lStatus=lst.id and lst.statusName= 'Enviado'"
    // . " order by lastName asc";

                            
                            $sql1 = "select * from lcm_contacts WHERE leadType = '{$produtoid1}' "; 

                         //   echo "<br>". $sql1;
                            
$contacts = $db->extQuery($sql1);
...
$objPHPExcel->getProperties()->setTitle("Leads CPTarget");
$objPHPExcel->getProperties()->setSubject("Leads Exportados CPTarget");
$objPHPExcel->getProperties()->setDescription("Leads Exportados CPTarget");
...
//l = 0;
    ...
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(19, $row, $val->customField);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(20, $row, $val->customField2);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(21, $row, $val->customField3);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(25, $row, $val->dateAdded);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(26, $row, $buceta);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(27, $row,$caralho);
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(23, $row, $noteMerge);
    $colStr = PHPExcel_Cell::stringFromColumnIndex(28);
...
if ($type == 'csv') {    // Save csv FILE
    ...
    header('Content-Disposition: attachment; filename=Leads-CPTarget.csv');
    ...
    header('Content-Disposition: attachment;filename=Leads-por-produto-CPTarget.xlsx');
      header('Content-Type: application/force-download');
                $objWriter->save('Leads-por-produto-CPTarget.xlsx');
 ...
 //    . " from {$dbPre}contacts c, {$dbPre}leadSource ls, {$dbPre}leadStatus lst, {$dbPre}leadType lt"
  //   . " where c.leadType=lt.typeID and c.leadSource=ls.sourceID and c.lStatus=lst.id and lst.statusName= 'Enviado'"
    // . " order by lastName asc";
            ...
            $sql1 = " SELECT * FROM lcm_contacts WHERE dateAdded BETWEEN '{$dataini}' AND '{$datafim}' ";       
...
if ($type == 'csv') {    // Save csv FILE
    ...
    header('Pragma: no-cache');
    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    header('Content-Disposition: attachment;filename=Leads-por-periodo-CPTarget.xlsx');
      header('Content-Type: application/force-download');
                $objWriter->save('Leads-por-periodo-CPTarget.xlsx');
...
if ($type == 'csv') {    // Save csv FILE
    ...
    header('Pragma: no-cache');
    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    header('Content-Disposition: attachment;filename=Leads-Enviados-CPTarget.xlsx');
                $objWriter->save('Leads-Enviados-CPTarget.xlsx');
...
$sql = "select c.*, ls.sourceName, lst.statusName, lt.typeName"
     . " from {$dbPre}contacts c, {$dbPre}leadSource ls, {$dbPre}leadStatus lst, {$dbPre}leadType lt"
     . " where c.leadType=lt.typeID and c.leadSource=ls.sourceID and c.lStatus=lst.id and ls.sourceName = '{$origem}'"
     . " order by lastName asc";
$contacts = $db->extQuery($sql);
...
if ($type == 'csv') {    // Save csv FILE
    ...
    header('Pragma: no-cache');
    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    header('Content-Disposition: attachment;filename=Leads-por-Origem-CPTarget.xlsx');
                $objWriter->save('Leads-por-Origem-CPTarget.xlsx');
}
