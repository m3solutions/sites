...
if (isset($_SESSION['token']) && isset($_POST['token']) && $_POST['token'] == $_SESSION['token']) {
    ...
    foreach ($_POST as $key => $value) {
        $key =$key;// trim(htmlentities($key, ENT_QUOTES, "iso-8859-1"));
        $value = $value;//trim(htmlentities($value, ENT_QUOTES, "iso-8859-1"));  // prior to 5.4 default is iso-8859-1
        $post[$key] = $value;
    }
    switch ($post['type']) {
        case 'login':                               // This is a standard login request
            ...
            if (isset($userRow[0]->userName)) {           // Successful login
                ...
                $jsonData['result'] = 0;
                $jsonData['text'] = 'Falha no Login Tente Novamente.';
        ...
        case 'saveLead':      // Save the lead
            ...
            if ($invalid == true) {
                ...
                if ($existing == 'existing') {          // Existing Lead update
                    ...
                    // add ao array o motivo do status 
                    $vals = array(
        ...
        case 'saveNote':                        // Save a note for a lead
            ...
            /*   commented out to use the preferred method below for encoding detection, this is all for m$ word pasting
            $note = str_replace(chr(130), ',', $note);    // baseline single quote
            $note = str_replace(chr(132), '"', $note);    // baseline double quote
            $note = str_replace(chr(133), '...', $note);  // ellipsis
            $note = str_replace(chr(145), "'", $note);    // left single quote
            $note = str_replace(chr(146), "'", $note);    // right single quote
            $note = str_replace(chr(147), '"', $note);    // left double quote
            $note = str_replace(chr(148), '"', $note);    // right double quote
            $note = str_replace(chr(226), "'", $note);    // some jacked up Microsoft single quote
            $note = str_replace(chr(194), '', $note);     //
            $note = str_replace(chr(128), '', $note);     // 
            $note = str_replace(chr(153), '', $note);     // Trademark symbol
        ...
        case 'saveSource':                  // Update a lead source
            ...
            $jsonData['result'] = 1;
            // logging
            $event = 'Origem Atualizada';
            $detail = "Nome Origem  $name";
            $log->logEvent($_SESSION['userID'], $event, $detail);
            break;
       ...
       case 'saveNewType':               // Save a new lead type
        ...
        case 'deleteType':                // Delete a type
            ...
            break;
                ...
                // incluir rotina para salvar motivo do status novoo
        
         case 'saveNewmotivo':               // Save a new lead motivo
            $name = $post['name'];
            $desc = $post['desc'];
            $vals = array (
                'status' => $name,
            ...
            $insertID = $db->insert("motivo_aux", $vals);
            $jsonData['result'] = 1;
            $jsonData['insertID'] = $insertID;
            $jsonData['vals'] = $vals;
            // logging
            $event = 'Novo motivo salvo';
            $detail = " Motivo : $name";
            $log->logEvent($_SESSION['userID'], $event, $detail);
            break;
       ...
       case 'saveNewStatus':               // Save a new lead status
        ...
        // incluir rotina para salvar motivo do status 
       
          case 'savemotivo':                  // Update a lead status
            $id = intval($post['id']);
            $name = $post['name'];
            $desc = $post['desc'];
            $where = array (
            ...
            $db->update("motivo_aux", $vals, $where);
            $jsonData['result'] = 1;
            // logging
            $event = 'Motivo atualizado';
            $detail = "Motivo $name";
            $log->logEvent($_SESSION['userID'], $event, $detail);
            break;
          ...
          //  } elseif (!filter_var($userEmail, FILTER_VALIDATE_EMAIL)) {
           //    $jsonData['result'] = 1;
               // $jsonData['msg'] = 'Invalid email address.';
                ...
                $detail = "Nome de usuário -  ". $userName;
                $log->logEvent($_SESSION['userID'], $event, $detail);
            }
            break;
        ...
        case 'updateRPP':                // Update Results per page
            ...
            $jsonData['rpp'] = $rpp;
            // logging
            $event = 'configurações atualizadas - resultados por página ';
            $detail = "$rpp";
            $log->logEvent($_SESSION['userID'], $event, $detail);
            break;
        ...
        case 'updateField':             // Update Field Names
            ...
            $jsonData['result'] = 1;
            // logging
            $event = 'Atualização de nome de campo - Field Name';
            $detail = "Alteração do nome  $field para $val";
            $log->logEvent($_SESSION['userID'], $event, $detail);
            break;
